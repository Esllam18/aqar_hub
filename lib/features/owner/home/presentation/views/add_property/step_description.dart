...
import 'package:aqar_hub/features/owner/home/presentation/widgets/ai_price_check_widget.dart';
...
class StepDescription extends StatelessWidget {
  ...
  const StepDescription({super.key, required this.formKey, required this.form, required this.onChanged});

  @override
  Widget build(BuildContext context) => stepScroll(context, Form(key: formKey,
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      StepAlertBanner(message: 'addprop_banner_description'.tr(context), type: AlertBannerType.success, icon: Icons.rocket_launch_outlined),
      SectionTitle('addprop_step_description'.tr(context)),
      HintText('addprop_description_hint'.tr(context)),
      inputField(context: context, hint: 'addprop_description_placeholder'.tr(context),
        initialValue: form.description, maxLines: 5,
        onChanged: (v) => onChanged(form.copyWith(description: v))),
      SizedBox(height: context.r(28)),
      AiPriceCheckWidget(form: form, onResult: (r) => onChanged(form.copyWith(aiPriceResult: r))),
    ])));
}
